...
The profile becomes a single column on phones. The navigation, research section links, and topic filters wrap. The Working Papers heading shares a row with Jump to links for Under Revision and Publications, wrapping on smaller screens. Selected filters have a filled state and a ring; keyboard focus is visible. Topic filters apply to all three publication groups. Empty groups and their section links are hidden; the jump navigation is hidden when neither destination has matches. A screen-reader announcement reports the active topic without a visible summary line. Printing includes the full paper list regardless of the selected filter.
